...
3. 다음 구조를 반드시 따르세요:
   ...
   - **Body 파트 2 (팩트체크 Q&A)**: 참여자가 10년차 베테랑 기자처럼 팩트 기반의 날카로운 질문을 던짐. 반드시 수치/데이터/비교를 요구하는 질문이어야 함. 사회자가 구체적 근거와 함께 심층 답변 (20-30초)
     - 좋은 질문 예: "근데 아빠, 방송 전에도 그 셰프들 레스토랑은 예약 잘 됐어요? 아니면 방송 효과로 갑자기 뛴 거예요?"
     - 나쁜 질문 예: "와 진짜요?" "대박이다!" (단순 감탄은 질문이 아님)
   - **Body 파트 3 (인사이트 Q&A)**: 참여자가 "그럼 결국..."으로 시작하는 심화 질문. Body 2 답변을 기반으로 영향/전망/숨은 의미를 파고듦. 사회자가 핵심 인사이트로 답변 (15-25초)
     - 좋은 질문 예: "그럼 결국 이런 프로그램 때문에 원래 동네 맛집들은 오히려 손해 보는 거 아니에요?"
     - 나쁜 질문 예: "저도 나중에 셰프 되고 싶어요!" (질문이 아닌 감상)
...
4. 장면별 image_prompt는 반드시 영어로 작성하세요 (DALL-E 3 용도).
   ...
   - 인물이 등장하는 장면은 **애니메이션/일러스트 스타일**로 묘사하세요.
   - **인물 나이 규칙**: 특정 실존 인물을 묘사하는 경우가 아니라면, 등장 인물은 반드시 **중년(40-50대)** 으로 묘사하세요.
     예: "anime-style middle-aged family watching news together" / "colorful animated middle-aged characters discussing technology"
...
- Body 파트 2 (팩트체크 Q&A): 참여자 질문 → 사회자 답변 형식 (scene_id: "body_2_1", "body_2_2", ...) — 참여자 질문은 10년차 기자처럼 원인/수치/비교를 파고드는 질문
- Body 파트 3 (인사이트 Q&A): 참여자 심화 질문 → 사회자 인사이트 답변 (scene_id: "body_3_1", "body_3_2", ...) — Body 2 답변을 바탕으로 영향/전망/반론을 제기하는 심화 질문
...
## 참여자 질문 작성 가이드 (매우 중요!)
참여자는 아이 캐릭터이지만, 질문 내용만큼은 10년차 베테랑 기자 수준이어야 합니다.
말투는 아이답게 유지하되, 질문의 핵심은 날카로워야 합니다.

### 필수 질문 기법 (Body 2, Body 3에서 반드시 사용):
1. **원인 추적**: "근데 그게 왜 그렇게 된 거예요?" (단순 "왜?"가 아닌 구체적 원인 질문)
2. **수치 요구**: "그럼 얼마나 늘어난 거예요? / 몇 % 차이예요?"
3. **비교 대조**: "그전에는 어땠는데요? / 다른 데는 안 그래요?"
4. **반론 제기**: "근데 그럼 ~한 문제는 없어요?"
5. **영향 확장**: "그럼 결국 ~한테도 영향 가는 거 아니에요?"

### 금지 패턴:
- 단순 감탄: "와!", "대박!", "진짜요?"만으로 끝나는 대사
- 개인 감상: "저도 ~하고 싶어요!" (질문이 아님)
- 정의 질문: "그게 뭐예요?" (Body 1에서만 허용)
- 반복 질문: 이전 장면과 비슷한 질문"""
...
- 인물이 필요한 장면은 **애니메이션/일러스트 스타일** 캐릭터로 묘사하세요.
- 특정 실존 인물을 묘사하는 경우가 아니라면, 인물은 반드시 **중년(middle-aged, 40-50대)** 으로 묘사하세요.

## 대본 작성 지침
- Body 1(아이스브레이킹): 위 최신 뉴스를 바탕으로 "왜 지금 이슈인지"를 쉽고 재미있게 설명하세요.
- Body 2(팩트체크 Q&A): 참여자가 10년차 기자처럼 원인/수치/비교를 파고드는 질문 → 사회자가 구체적 데이터와 근거로 답변. 단순 감탄("와!", "대박!") 금지.
- Body 3(인사이트 Q&A): Body 2 답변을 기반으로 영향/전망/반론을 제기하는 심화 질문 → 사회자가 핵심 인사이트로 답변. 개인 감상("저도 ~하고 싶어요") 금지.
...
- **참여자 질문의 품질**: Body 2~3에서 참여자 질문이 원인/수치/비교/반론/영향 중 하나 이상을 다루는가
- **단순 감탄 비율**: 참여자 대사 중 질문 없이 감탄만 하는 장면이 30% 이하인가"""
